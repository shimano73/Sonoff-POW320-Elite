...
void CSE_7766::readValuesFromDevice() {
  ...
  if (relayChannel) {
    currentChanelRelay = relayChannel->getValueBool();
  }
  ...
  // zapis do naszej struktury (dla wyświetlacza itp.)
  measurement.voltage = _voltage;
  measurement.current = _current;
  measurement.power   = _active;
  measurement.energy  = energy / 100000.0;
  measurement.valid   = true;

  // przekazanie danych do SUPLA
  setVoltage(0, _voltage * 100);            // 0.01 V
  setCurrent(0, _current * 1000);           // 0.001 A
  setPowerActive(0, _active * 100000);      // 0.00001 kW
  setFwdActEnergy(0, energy);               // 0.00001 kWh
  setPowerApparent(0, _apparent * 100000);  // 0.00001 kVA
  setPowerReactive(0, _reactive * 100000);  // 0.00001 kvar
  setPowerFactor(0, _pf * 1000);            // 0.001
}
